...
you can use the `pixelsToMatrices()` function, where the
`buffer` argument describes the number of pixels to
expand the ranges on either side of the center pixel.
For example, since the `gi`...

```{r}
gi |> width()
pixelsToMatrices(x=gi, buffer=2) |> width()
```
